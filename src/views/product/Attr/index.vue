    ...
    <el-card>
      <div v-show="isShowTable">
        ...
        <el-table style="width: 100%;" border :data="attrList">
          ...
          <el-table-column prop="prop" label="属性值列" width="width">
...
export default {
  ...
  methods: {
    ...
    addAttrValue() {
      // 向属性值的数组里面添加元素
      // attrId:是你相应的属性的id,目前而言我们是添加属性的操作，还没有相应的属性的id,目前而言带个服务器的id为undefined
      // valueName:相应的属性值的名称
      this.attrInfo.attrValueList.push({
        attrId: this.attrInfo.id, //对于修改某一个属性的时候，
        valueName: '',
        flag: true,
      });
    ...
    updateAttr(row) {
      ...
      this.isShowTable = false
      // 将选择的属性赋值给attrList
      // 由于数据结构当中存在对象里面套数组，数组里面有对象，因此需要使用深拷贝解决这类问题
    ...
    async addOrUpdateAttr() {
      ...
      try {
        ...
        //提示消失
        this.$message({ type: 'succes', message: '保存成功' });
        //  保存成功后需要再次获取平台属性列表
        this.getAttrList();
      } catch (error) {
        this.$message('保存失败')
    ...
    async deleteAttr(row){
      ...
      if(result.code==200){
        ...
        //代表attr个数大于1删除的时候停留在当前页，如果attr个数小于1 回到上一页
        this.getAttrList();
      }
    }
  },
