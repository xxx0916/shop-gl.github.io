<template>
  <div>
    <!-- 按钮 -->
    <el-button type="primary" icon="el-icon-plus" style="margin:10px 0px ;" @click="updateTradeMark">添加</el-button>
    <!-- 表格组件
    ...
    <el-dialog title="添加品牌" :visible.sync="dialogFormVisible">
      <!-- form表单 model属性作用：把表单的数据收集到哪个对象的身上，将来表单验证，也需要这个属性-->
  ...
  data() {
    ...
    var validateTmName = (rule, value, callback) => {
      ...
      if (value.length < 2 || value.length > 10) {
        callback(new Error('品牌名称的长度2-10'))
      } else {
        callback();
      }
    };
    return {
      ...
      rules: {
        // 品牌名称的验证规则
        // required:必须要检验的字段（前面五角星有关系） message 提示信息  trigger 用户行为设置（事件设置blur\change）
        tmName: [
